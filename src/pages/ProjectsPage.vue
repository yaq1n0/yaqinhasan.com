<template>
  <carousel-layout title="My Projects" :items="projectItems" :initial-item-id="activeProject">
    <template #default="{ activeItem }">
      <!-- Website Project -->
      <project-card :title="projects.website.title" :summary="projects.website.summary" :visible="activeItem.id === 'website'">
      ...
      <!-- flopPy Project -->
      <project-card :title="projects.floppy.title" :summary="projects.floppy.summary" :visible="activeItem.id === 'floppy'">
        <p>
      ...
      <!-- RoboCopyGUI Project -->
      <project-card :title="projects.robocopy.title" :summary="projects.robocopy.summary" :visible="activeItem.id === 'robocopy'">
        <p>
          I couldn't find a lightweight local file backup solution to backup my main Windows PC to my SMB NAS, so I decided to look around. I found RoboCopy, a
          windows CLI tool which provides the backend functionality for this. I then wanted to make a GUI for this so that I can use this in a bit more refined
          way.
        </p>

        <h4>Flutter Solution</h4>
        <p>
          The current solution is a Flutter windows app, which I chose because I was currently learning how to use Flutter at the time. This solution is around
          50% complete, as it does work, but the layout is nonexistent and it's not very thoroughly tested. You can find this in the repo linked above.
        </p>
        ...
        <p>
          ...
          make it even better for the intended purpose, as it could then be unintrusively launched at startup and stay in the background. With the option to be
          manually run when needed and opened up to configure settings.
        </p>
      </project-card>

      <!-- Tracker Project -->
      <project-card :title="projects.tracker.title" :summary="projects.tracker.summary" :visible="activeItem.id === 'tracker'">
        <p>
          I noticed a market gap for a certain kind of tracker app on the Google Play Store, I then decided to come up with a rough design for an app that could
          fill that niche. This project is highly speculative at this point.
        </p>
        <p>I will write more here once I have more implementation details.</p>
      </project-card>

      <!-- ThymPi Project -->
      <project-card :title="projects.thympi.title" :summary="projects.thympi.summary" :visible="activeItem.id === 'thympi'">
        ...
        <template #dev-content>
          ...
          <p>
            I would have needed some kind of mobile robotic test platform to develop the prototype on. In theory, I could've used anything that had the ability
            to move, a couple servos and wheels bolted to a slab.
          </p>
          ...
          <p>
            Up until this project, I had done some modules on ML theory, but I have never actually implemented or used anything in a practical setting. I did a
            lot of research and found out about OpenCV, which allowed me to use pre-trained object classification models to detect and classify objects. I
            eventually settled on using a model specifically for "everyday objects" which I believed would allow me to classify anything I would use in testing.
          </p>

          <p>
            Obviously running in object recognition model on a Raspberry Pi 3 single board computer was going to run into some performance constraints. I tried
            my best to optimize by disabling the preview and directly sending the camera input to the model. Eventually I had to sacrifice the input resolution
            to get the model to work in somewhat real-time, which meant less accurate object classification. However, I managed to get around this by caching
            the recognition data once and using it for subsequent frames. The final solution ended up with minimal recognition delay in the few hundred
            milliseconds range, but also being consistent and accurate enough practically.
          </p>

          <h4>Theoretical Improvements</h4>
          <p>
            By calculating compliance_value from f(internal_variable), we are mapping a single sensor's data to a numerical value. This inherently has 2 issues,
            the first being that information is lost during the mapping process, regardless of what function is used. Secondly, using a single sensor value is
            inherently limiting, as it can often miss certain nuances.
          </p>

          <p>
            On the function side, the simplest function used to turn a range of values, such as deceleration, into a single value is an average. This would lead
            to a sharp single spike such as when colliding with a completely immovable and non-compliant obstacle to return a lower compliance_value than a
            prolonged deceleration as as from pushing a heavy but movable and slightly compliant obstacle.
          </p>
      ...
      <!-- STQL Project -->
      <project-card :title="projects.stql.title" :summary="projects.stql.summary" :visible="activeItem.id === 'stql'">
        ...
        <p>
          The second thing I realized was that the actual functionality could be implemented using a combination of simpler functions rather than using a larger
          set of more complex functions. Essentially reducing the interpreter complexity at the "cost" of requiring more effort to write the code in the Toy
          language.
        </p>
      </project-card>

      <!-- SEGP Project -->
      <project-card :title="projects.segp.title" :summary="projects.segp.summary" :visible="activeItem.id === 'segp'">
        <p>
