...
const default_item = "project0"; 
if (sessionStorage.getItem("projectsCurrent") == null) {
    sessionStorage.setItem("projectsCurrent", default_item);
}
...
function cycleItem(item_id) {
  ...
  sessionStorage.setItem("projectsCurrent", item_id); // set current item in sessionStorage
}

// go to the previous item, handle loopback
function previousItem() {
  let current = sessionStorage.getItem("projectsCurrent");
  if (current != null) {
    let current_id = parseInt(current.replace("project", ""));
    let previous_id = current_id - 1;
    if (previous_id < 0) previous_id = carousel_count;
    cycleItem("project" + previous_id);
  }
}
...
function nextItem() {
    ...
    if (current != null) {
      ...
      cycleItem("project" + next_id);
    }
  }

// cycle the item to the currentproject on window load
window.addEventListener('load', cycleItem(sessionStorage.getItem("projectsCurrent")));
